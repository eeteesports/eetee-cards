// Shared eBay helper
// Priority: Finding API (sold) → Browse API (active listings)
...
// Finding API — original sold comps endpoint (App ID only, no OAuth needed)
async function fetchFindingAPISold(keywords) {
  ...
  if (text.trimStart().startsWith('<')) return null // HTML = API down or keyset disabled
...
export async function fetchEbayComps(player, year, set, brand, parallel, condition, printRun) {
  ...
  try {
    // 1. Try Finding API (sold) — works if keyset is active, no OAuth needed
    const findingSold = await fetchFindingAPISold(keywords)
    if (findingSold && findingSold.length > 0) {
      return { keywords, items: findingSold, dataType: 'sold' }
    }

    // 2. OAuth token for REST API
    const token = await getToken()

    // 3. Try Marketplace Insights REST (sold) — only if scope available
    ...
    // 4. Fall back to Browse API (active listings)
